...
- **短期记忆** — 每个聊天独立的对话历史（10 轮，2 小时超时自动清空）
- **长期记忆** — 持久化 `/mem` 系统，带 token 统计，以 Markdown 文件存储
- **日志自动归档** — 每日对话自动记录→次日 LLM 提炼→5 天压缩为周报
...
每个聊天独立维护对话历史，最多 10 轮。空闲 2 小时后自动清空。始终开启，用于保持多轮对话连贯性。
...
所有对话自动记录到 `data/logs/YYYY-MM-DD.txt`（纯文本，零 token 开销），独立于短期对话历史运行。

**自动流水线：**

1. **每日记录** — 每条消息追加写入当日日志文件
2. **次日提炼** — 新一天的首条消息触发时，将前一天的日志发给 LLM 提炼（≤150 字），存为 `daily:YYYY-MM-DD` 记忆条目
3. **周期压缩** — 日报累积超过 5 条时，自动合并为一条 `weekly:` 周报（≤300 字）

**手动命令：**

```
/remember [提示] — 提炼当前对话要点存入记忆（可带聚焦提示）
/log             — 查看日志系统状态
